...
class Channel extends Component {
    constructor (props) {
        ...
        this.state = {
            ...
            trackerList: [],                    // 催服专员
            defaultTrackerId: null,             // 当前的催服专员Id员Id
        };
    }
    ...
    async getTrackerList () {
        ...
        if (data.success) {
            ...
            content.forEach(item => {
                if (item.trackerMobile === Cookies.get('m')) {
            ...
            this.setState({
                trackerList: arr,
                defaultTrackerId: currentTrackerId
            }, () => {
                currentTrackerId && this.getList();
            });
        }
    }
    ...
    async getList () {
        ...
        const {defaultTrackerId} = this.state;
        if (!defaultTrackerId) {
            message.info(`您当前还不是催服专员，暂无数据可查询。请联系管理员`);
            return;
        }
        this.resetItem();
        const params = Object.assign({}, this.state.pagination, this.state.searchParams);
        delete params.total;
        params.trackerId = defaultTrackerId;
    ...
    async getAllPaymentAmountTotal () {
        ...
        searchParams.trackerId = defaultTrackerId;
        this.setState({
    ...
    render () {
        ...
        const {
            ...
            editMyCaseCode,
            deleteMyCasePower,
            sendMessageMyCasePower,
            uploadMyCasePower,
            appointMyCasePower,
            withholdingMyCasePower,
            caseDetail
        ...
        return (
            <div>
                <div className="m-b-xs">
                    <Search search={searchItem} onChange={this.handleSearch.bind(this)} onReset={this.handleReset.bind(this)} background={'#e6f7ff'}/>
                </div>
                <div>
                    <JycCard>
                        <div className="m-b-sm">
                            {importMyCaseCode && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.openImportModal}>导入</Button>}
                            {outputMyCaseCode && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.exportList}>导出</Button>}
                            {/*{<Button className="m-r-xs m-t-xs" type="primary" onClick={this.openPenaltyAmountModal}>罚息减免</Button>}*/}
                            {editMyCaseCode && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.openEditModal}>编辑</Button>}
                            {deleteMyCasePower && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.deleteItem}>删除</Button>}
                            {/*{mySeparateModalCode && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.openSeparateModal.bind(this)}>分单</Button>}*/}
                            {myTrackListCode && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.goTrackList}>催记管理</Button>}
                            {sendMessageMyCasePower && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.openBatchSendMessage}>批量发送短信</Button>}
                            {uploadMyCasePower && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.openUploadModal}>上传附件</Button>}
                            {appointMyCasePower && <Button className="m-r-xs m-t-xs" type="primary" onClick={this.openAppointCaseModal}>变更委案状态</Button>}
                            {withholdingMyCasePower && <Button className='m-t-xs' type="primary" onClick={this.openWithholding}>发起代扣</Button>}
                        </div>
                        <TablePage
                            rowKey={'id'}
                            data={pageList}
