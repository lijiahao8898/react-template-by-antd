...
class SeparateModal extends React.Component {
    ...
    async modalOk () {
        this.props.form.validateFields(async (err, value) => {
            if (!err) {
                const borrowerId = this.getBorrowerId(this.props.selectedRows);
                this.setState({
                    operateLoading: true
                })
    ...
    getBorrowerId (selectedRows) {
        let borrowerId = [];

        selectedRows.forEach(item => {
            if(!borrowerId.includes(item.borrowerId)) {
                borrowerId.push(item.borrowerId)
            }
        });
    ...
    getTotalAmount (selectedRows) {
        let totalAmount = 0;
        selectedRows.forEach(item => {
            totalAmount = accAdds(totalAmount, item.investorPrincipal, item.investorInterest, item.repaymentAmount);
        });
